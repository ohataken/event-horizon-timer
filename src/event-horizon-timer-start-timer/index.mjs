...
/**
 * event-horizon-timer-start-timer
...
export async function handler(event) {
  ...
  try {
    ...
    const targetTime = Date.now() + timerResult.Item.duration * 1000;

    const result = await dynamoDB.send(new UpdateCommand({
      TableName: tableName,
      Key: { id },
      UpdateExpression: "SET #status = :status, #target_time = :target_time",
    ...
    await Promise.all(clientIds.map(async (clientId) => {
      ...
      try {
        await apiGw.send(new PostToConnectionCommand({
          ...
          Data: Buffer.from(JSON.stringify({ action: "start_timer", timer: timerResponse }))
        }));
      } catch (err) {
    ...
    return {
      ...
      body: JSON.stringify({ message: `Failed to start timer ${error}` })
    };
  }
}
